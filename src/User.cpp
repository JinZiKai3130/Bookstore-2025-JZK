...
UserManager::UserManager() : user_storage("Users.dat") {
...
void UserManager::useradd(const char *id, const char *pwd, int priv,
                          const char *name, const int your_priv,
                          const bool modify) {
  // std::cout << "cur_priv = " << your_priv << '\n';
  if (!check_priv(priv) || !check_id(id) || !check_id(pwd) ||
      !check_name(name)) {
...
void UserManager::login(const char *id, const char *pwd) {
  int your_priv;
  (user_stack.empty()) ? (your_priv = 0)
                       : (your_priv = user_stack.back().privilege);
...
void UserManager::change_pwd(const char *id, const char *new_pwd, int your_priv,
                             const char *pre_pwd) {
  ...
  UserManager::useradd(id, new_pwd, cur_user[0].privilege, cur_user[0].username,
                       7, 1);
}
