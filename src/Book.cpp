...
BookManager::BookManager()
    : book_storage("book_storage.dat"), name_isbn("name_isbn.dat"),
      author_isbn("author_isbn.dat"), keyword_isbn("keyword_isbn.dat") {}
...
void BookManager::insert(const Book &new_book) {
  ...
  if (strlen(new_book.keyword) != 0) {
    ...
    for (const auto &kwd : kwds) {
      ...
      keyword_isbn.add_data(new_keyword,
                            keyword_isbn_(new_keyword, new_book.ISBN));
    }
  }
}
...
void BookManager::dele(const char *isbn) {
  ...
  // std::cout << "cur_book[0].name = " << cur_book[0].name << "end" <<
  // std::endl;
  ...
  // std::cout << "cur_book[0].author = " << cur_book[0].author << "end" <<
  // std::endl;
  if (strlen(cur_book[0].author) != 0)
    author_isbn.remove_data(cur_book[0].author, cur_author);
  // std::cout << "cur_book[0].keyword = " << cur_book[0].keyword.empty() << "
  // end" << std::endl;
  if (strlen(cur_book[0].keyword) != 0) {
    std::vector<std::array<char, 61>> kwds =
        parse_keywords(cur_book[0].keyword);
    for (const auto &kwd : kwds) {
      char new_keyword[61];
      std::copy_n(kwd.begin(), 61, new_keyword);
      keyword_isbn.remove_data(new_keyword,
                               keyword_isbn_(new_keyword, cur_book[0].ISBN));
    }
  }
  // std::cout << "go here\n";
}
...
vector<std::array<char, 61>>
BookManager::parse_keywords(const string &keyword_str) {
  ...
  while (end <= keyword_str.length()) {
    if (end == keyword_str.length() || keyword_str[end] == '|') {
      ...
      if (find(temp_keywords.begin(), temp_keywords.end(), keyword) !=
          temp_keywords.end()) {
        throw("Invalid\n");
      }
...
void BookManager::show(const string &str) {
  ...
  if (s == "keyword") {
    for (int i = pos + 1; i < str.length(); i++) { // no many keywords together
      if (str[i] == '|') {
        throw("Invalid\n");
      }
    }
